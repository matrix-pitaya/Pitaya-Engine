...
1.  **克隆仓库:**
    ...
    下载GitHub仓库Release中的 `Pitaya-Engine-res-third-vx.x.x.zip` 压缩包并解压缩 `thirdparty` 文件夹到`Pitaya.sln`目录。
...
4.  **运行项目:**
    右键重新生成解决方案并将 `resource` 文件夹和以下 `dll` 拷贝到 `exe` 目录下，返回 `VS` 将 `Application` 设为启动项目并按 `F5` 运行。
    
    ```
    thirdparty/mimalloc/dll  中的  mimalloc.dll 和 mimalloc-redirect.dll
    thirdparty/GLEW/dll      中的  glew32.dll
    thirdparty/GLFW/dll      中的  glfw3.dll 
    ```
