...
namespace Pitaya::Game
{
	...
	public:
		...
		SceneECS(SceneECS&&) = delete;
		SceneECS& operator=(SceneECS&&) = delete;
	...
	public:
		...
		inline void SetRelationship(entt::entity childId, entt::entity parentId, entt::entity afterSibling = entt::null, bool keepWorldTransform = true)
		{
			...
			{
				metadata->MarkDirty(Pitaya::Core::PassKey<SceneECS>());
			}
		}
		inline void ReserveDestroyQueue(size_t capacity) 
		{ 
			delayDestroyQueue.reserve(capacity);
		}
	...
	public:
		inline void ProcessDelayDestroyQueue(Pitaya::Core::PassKey<GameWorld>)
		{
		...
		inline void ProcessHierarchySystem(Pitaya::Core::PassKey<GameWorld>)
		{
	...
	private:
		inline void UpdateHierarchy(entt::entity entity, const glm::mat4& parentWorldMatrix, bool parentChanged, bool parentActive, bool parentActiveChanged)
		{
			...
			if (activeNeedsUpdate)
			{
				...
				if (metadata && activeDirty) { metadata->MarkClear(Pitaya::Core::PassKey<SceneECS>()); }
			}
