...
namespace Pitaya::GPU
{
	...
	public:
		template<GPUObjectType T, typename... Args>
		inline auto Create(Pitaya::Core::PassKey<Pitaya::Render::Renderer>, Args&&... args) 
		{
			return registry.Create<T>(std::forward<Args>(args)...);
		}
		template<GPUObjectType T>
		inline bool Destroy(Pitaya::Core::PassKey<Pitaya::Render::Renderer>,
			typename Pitaya::Core::SlotMap<T>::Handle handle) 
		{
			return registry.Destroy<T>(handle);
		}
		template<GPUObjectType T>
		inline bool Get(Pitaya::Core::PassKey<Pitaya::Render::Renderer>,
			typename Pitaya::Core::SlotMap<T>::Handle handle, T& out) const 
		{
